...
The nmap scan detection rule worked well in testing, even detecting nmap's slowest scan rate(-T1). Although it consistently generated alerts for nmap scans, adding a simple scan delay allowed nmap to evade detection. There are many other nmap scan options like fragmentation of packets or decoys that would allow a port scan to escape detection. Also, the low detection threshold(5 SYN packets in 60 seconds) is good for identifying patient attackers, but the opening of other TCP ports could potentially generate false positives. 
